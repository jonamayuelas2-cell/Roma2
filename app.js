        ...
        callGlobe('htmlElement', d => {
            ...
            el.innerHTML = `
                <div class="globe-thumb-container">
                    ...
                    ${!isStop ? `<span class="globe-emoji globe-iata">${escapeHtml(getCityIataCode(d))}</span>` : ''}
                </div>
            `;
            el.onclick = (ev) => { 
                ev.stopPropagation(); 
...
function renderPlaceActivities(place) {
    ...
    container.innerHTML = activities.map(activity => `
        <article class="activity-card">
            <img src="${assetUrl(activity.image)}" alt="${escapeHtml(activity.title)}" onerror="this.onerror=null;this.src='${assetUrl(place.imagenCard || place.imagen)}';">
